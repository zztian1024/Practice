...
static double kDefaultInterval = 1;
...
+ (void)ci_exchangeClickMethod {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        //  获得方法选择器
        SEL originalSel = @selector(sendAction:to:forEvent:);
        SEL newSel = @selector(_sendClickIntervalAction:to:forEvent:);
        //获得方法
...
- (void)_sendClickIntervalAction:(SEL)action to:(id)target forEvent:(UIEvent *)event {
    if ([self isKindOfClass:[UIButton class]] && !self.ignoreClickInterval) {
        ...
        if (self.clickInterval > 0) {
            ...
            [self performSelector:@selector(ci_ignoreClickState:)
                       withObject:@(NO)
                       afterDelay:self.clickInterval];
        }
    }
    [self _sendClickIntervalAction:action to:target forEvent:event];
}

- (void)ci_ignoreClickState:(NSNumber *)ignoreClickState {
