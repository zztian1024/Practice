...
@implementation NWLogger

// Lifetime

- (instancetype)initWithLoggingBehavior:(NSString *)loggingBehavior {
    if ((self = [super init])) {
        _active = [NWSettings.loggingBehaviors containsObject:loggingBehavior];
        _loggingBehavior = loggingBehavior;
        if (_active) {
            _internalContents = [[NSMutableString alloc] init];
            _loggerSerialNumber = [NWLogger generateSerialNumber];
        }
    }
...
+ (void)singleShotLogEntry:(NSString *)loggingBehavior
    ...
    if ([NWSettings.loggingBehaviors containsObject:loggingBehavior]) {
        NWLogger *logger = [[NWLogger alloc] initWithLoggingBehavior:loggingBehavior];
...
+ (void)singleShotLogEntry:(NSString *)loggingBehavior
    ...
    if ([NWSettings.loggingBehaviors containsObject:loggingBehavior]) {
        ...
        if (startTimeNumber != nil) {
            uint64_t elapsed = [UtilityTools currentTimeInMilliseconds] - startTimeNumber.unsignedLongLongValue;
...
+ (void)registerCurrentTime:(NSString *)loggingBehavior
    ...
    if ([NWSettings.loggingBehaviors containsObject:loggingBehavior]) {
        ...
        if (g_startTimesWithTags.count >= 1000) {
            [NWLogger singleShotLogEntry:NWLoggingBehaviorDeveloperErrors logEntry:
             @"Unexpectedly large number of outstanding perf logging start times, something is likely wrong."];
        }
        
        uint64_t currTime = [UtilityTools currentTimeInMilliseconds];
        
        // Treat the incoming object tag simply as an address, since it's only used to identify during lifetime.  If
        // we send in as an object, the dictionary will try to copy it.
        unsigned long tagAsNumber = (unsigned long)(__bridge void *)timestampTag;
        [g_startTimesWithTags safe_setObject:@(currTime) forKey:@(tagAsNumber)];
    }
}
...
+ (void)registerStringToReplace:(NSString *)replace
    ...
    if (NWSettings.loggingBehaviors.count > 0) { // otherwise there's no logging.
